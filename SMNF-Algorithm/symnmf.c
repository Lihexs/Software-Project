...
int main(int argc, char* argv[]){
   char* goal = argv[1]; /* Which matrix to compute */
   char* filepath = argv[2]; /* Input file */
   int num_of_coordinates; /* Size of each datapoint */
   int N; /* Number of datapoints */
   double** X = NULL; /* The datapoints array */
   double** result; /* The output */
   (void)argc;
   if (initialize(&X, &num_of_coordinates, &N, filepath) != 0){ /* Initialize the passed arguments*/
        printf("An Error Has Occurred");
        return 1;
   }
   
   if (!strcmp(goal, "sym")){ /* If goal equals sym, apply sym_c */
      if ((result = sym_c(X,num_of_coordinates,N)) == NULL){
         printf("An Error Has Occurred");
         return 1;
      }
   }
   else if (!strcmp(goal, "ddg")){ /* If goal equals ddg, apply ddg_c*/
      ...
      if ((result = norm_c(X,num_of_coordinates,N)) == NULL){ /* If goal equals norm, apply norm_c */
         printf("An Error Has Occurred");
         return 1;
      }
   }    
   print_matrix(result, N, N);
   free_matrix(X, N); /* Free the memory allocated */
...
int initialize(double *** arr_ptr, int * num_of_coordinates_ptr, int * N_ptr, char* filepath) {
   ...
   while ((nread = getline(&line, &len, file)) != (size_t)-1){
      ...
      for (i = 0; i < nread; i++) { /* Computing num_of_coordinates */
...
int chr_until_comma(const char * str){ /* Computing the number of characters until we find a comma */
    int i = 0;
    if (str == NULL){
        return 0;
    }
